...
import { Plus, Search, Database, FileText, LogOut, HardDrive, Trash2, Lock, Globe } from 'lucide-react';
...
interface BucketSidebarProps {
  ...
  onToggleBucketAccess: (bucket: Bucket) => void;
  onLogout: () => void;
}
...
export function BucketSidebar({
  ...
  return (
    <>
          ...
          {filteredBuckets.map((bucket) => (
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onToggleBucketAccess(bucket);
                }}
                className="opacity-0 group-hover:opacity-100 p-1 hover:bg-muted rounded transition-opacity"
                title={bucket.access === 'PUBLIC' ? 'Make Private' : 'Make Public'}
              >
      ...
      <DocumentationOverlay open={docsOpen} onClose={() => setDocsOpen(false)} />
    </>
  );
}
